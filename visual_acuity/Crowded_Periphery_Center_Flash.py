...
if recordData:
    ...
    expName = 'Crowded Periphery Center Flash'
...
def instructions():
    ...
    genDisplay({'text': 'at the center of the plus (e, p, b),',\
...
def angleCalc(angle):
    ...
    pos = math.tan(radians)*distToScreen 
    return pos
...
def calculateDisplayCoords(trialInfo): 
    ...
    displayInfo['spacer'] = spacer*sizeCm
    
    displayInfo['heightCm'] = sizeCm*heightMult
    
    # Stimulus display x position
    if trialInfo['dir'] == 0:
        displayInfo['xPos'] = centerX + (angleCm*rightXMult)
    elif trialInfo['dir'] == 2:
        displayInfo['xPos'] = centerX + (angleCm*leftXMult)
    
    # Set color to white
    displayInfo['color'] = 'white'
    return displayInfo
    
def calculateYCoord(displayInfo):
    displayInfo['yPos'] = displayInfo['spacer']*displayInfo['rowIdx']
    return displayInfo
...
#  B
#
def genArray(trialInfo):
    
    displayInfo = calculateDisplayCoords(trialInfo)
    ...
    for i in range(rows):
        ...
        displayInfo = calculateYCoord(displayInfo)

        if i == 1: #Creates new masking object
            displayInfo['text'] = centerChar
            centerMask = genDisplay(displayInfo)
            
        
        # Convert the list to a string, create a display object, and draw it
        displayInfo['text'] = ''.join(line)
        
        
        genDisplay(displayInfo).draw()
    return centerChar, centerMask
...
def expBreak():
    breakText = genDisplay({'text': 'Break', 'xPos': 0, 'yPos': 4, 'heightCm': 3, 'color': 'white'})
...
for pair in pairs: # Loop through the list of pairs
    ...
    while not trialInfo['stairCaseCompleted']:
        ...
        centerChar, centerMask = genArray(trialInfo)
        ...
        while 1:
            ...
            if flash:
                dot.color = 'lawngreen' # Draw the green dot
                centerMask.color = 'white'
            else:
                dot.color = 'grey'
                centerMask.color = 'grey'
            
            dot.draw() # Draw the dot
            centerMask.draw()
            
            win.flip(clearBuffer = False) # Update the display
        ...
        trialInfo['thisResponse'] = checkResponse(response, centerChar)
...
genDisplay({'text': 'Experiment complete!',\
        'xPos': 0, 'yPos': 0, 'heightCm': 1.5, 'color': 'white'}).draw()
win.flip()
time.sleep(3)
